...
function initGame() { // function for initiating the game
    ...
    for (var i = 0; i < cards.length; i++) { // loop for deck elements "li"
        deck.append($('<li class="card yetToOpen"><i class="fa fa-' + cards[i] + '"></i></li>')) // "li" appended with element "i" and symbols
    };
...
var CardListener = function () {
    ...
    deck.find('.yetToOpen').bind('click', function () { // followup function to when no class match or open
        var $this = $(this),
            card = $this[0].innerHTML; // assign innerHTML of the selected element to card
            var test = $this[0].classList[1];

            console.log($this);
 
        $this.addClass('open show'); // required class added
        $this.removeClass("yetToOpen") //
        ...
        if (opened.length > 1) { // if some element in opened array, perform this
            ...
            var openedCard = $this[0].classList[1];

            if (card === opened[0]) {
                deck.find('.open').addClass('match grow gone'); // add class when element is matched with elements in opened array
